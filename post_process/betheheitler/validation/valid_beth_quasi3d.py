...
case = ["betheheitler_phot_10MeV.2d/", "betheheitler_phot_10MeV.q3d/"]
...
for i_case in range(N_case):
    ...
    N = 400
    dNdp_raw[i_case], bin_edges = np.histogram(data_raw['gamma'], bins=N, range=(min_p, max_p))
    ...
    M = 10000
    gamp_the[i_case] = np.linspace(min_p, max_p, M)
    dNdp_the[i_case] = np.array([qed_rates.bh_cs_dif(elem, k, Z) for elem in gamp_the[i_case]])
...
for i_case in range(N_case):
    axs.plot(gamp_raw[i_case]/k, dNdp_raw[i_case], label=labels[i_case])
axs.plot(gamp_the[0]/k, dNdp_the[0], c='k', label=r"$\rm theory $")
axs.set_xlabel(r"$ \gamma_+ / k $")
axs.set_ylabel(r"$ dN/d\gamma_+ $")
plt.legend(loc='best')
plt.tight_layout()
filename = 'pics/valid_beth_quasi3d.png'
